...
func TestSchemaTypeValidation(t *testing.T) {
	...
	if _, errs := schema.Validate("abc", self, nil, nil, path); errs != nil {
		t.Error("Should pass when value is correct type")
	}

	if _, errs := schema.Validate(123, self, nil, nil, path); errs == nil {
...
func TestSchemaArrayValidation(t *testing.T) {
	...
	if _, errs := schema.Validate([]interface{}{"abc"}, self, nil, nil, path); errs != nil {
		t.Error("Should pass when value is an array of the correct type")
	}

	if _, errs := schema.Validate([]interface{}{"abc", 123}, self, nil, nil, path); errs == nil {
		t.Error("Should fail when value is a mixed array")
	}

	if _, errs := schema.Validate([]interface{}{123}, self, nil, nil, path); errs == nil {
...
func TestSchemaCustomValidation(t *testing.T) {
	...
	if _, errs := schema.Validate(float64(12), self, template, definitions, path); errs != nil {
		t.Error("Should run custom validation when type is correct", errs)
	}

	if _, errs := schema.Validate(float64(20), self, template, definitions, path); errs == nil {
		t.Error("Should run custom validation when type is correct")
	}

	if _, errs := schema.Validate("abc", self, template, definitions, path); errs != nil && errs[0].Message != "Property has invalid type string, expected: ValueNumber" {
		t.Error("Should not run validation when type is correct", errs)
	}

	if _, errs := schema.Validate(map[string]interface{}{"Ref": "abc"}, self, template, definitions, path); errs != nil {
