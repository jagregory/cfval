...
func TestPeriodValidation(t *testing.T) {
	...
	if _, errs := period.Validate(schema.Schema{}, "", tr, context); errs == nil {
		t.Error("Period should fail on empty string")
	}

	if _, errs := period.Validate(schema.Schema{}, "abc", tr, context); errs == nil {
		t.Error("Period should fail on anything which isn't a period")
	}

	for _, ex := range []string{"0", "10", "119", "260"} {
		if _, errs := period.Validate(schema.Schema{}, ex, tr, context); errs == nil {
