...
func TestAutomaticFailoverEnabled(t *testing.T) {
	...
	if _, errs := automaticFailoverEnabled(schema.Schema{}, true, badVersion, context); errs == nil {
		t.Error("Should fail if has engine less than 2.8")
	}

	if _, errs := automaticFailoverEnabled(schema.Schema{}, true, badNodeTypeT1, context); errs == nil {
		t.Error("Should fail if has node type of t1 or t2")
	}

	if _, errs := automaticFailoverEnabled(schema.Schema{}, true, badNodeTypeT2, context); errs == nil {
		t.Error("Should fail if has node type of t1 or t2")
	}

	if _, errs := automaticFailoverEnabled(schema.Schema{}, true, good, context); errs != nil {
